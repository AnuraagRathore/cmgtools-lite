...
BCORE=" --s2v --tree ${TREE} ttH-multilepton/lepton-fr/lepton-mca-frstudies.txt object-studies/lepton-perlep.txt  --lumi ${L} "
BCORE="${BCORE} -L ttH-multilepton/functionsTTH.cc  --year ${YEAR} "
...
BG=" -j 16 "; if [[ "$1" == "-b" ]]; then BG=" -j 4 & "; shift; fi
...
for WP in $WPs; do
        ...
        VDFL="LepGood_jetBTagDeepFlav < btagcutL"
        VDFM="LepGood_jetBTagDeepFlav < btagcutM"
        VDFT="LepGood_jetBTagDeepFlav < btagcutT"

        PTF30="1/(1+LepGood_jetRelIso) > 0.3"
	VETOCONVERSIONS="LepGood_mcPromptGamma==0"
        case $WP in 
            000*) WNUM="0.00" ;; 030*) WNUM="0.30" ;; 060*) WNUM="0.60" ;;
            075*) WNUM="0.75" ;; 080*) WNUM="0.80" ;; 085*) WNUM="0.85" ;;  090*) WNUM="0.90" ;; 064*) WNUM="0.64";;
	    sM*) WNUM="if3(abs(LepGood_pdgId)==13,-0.2,0.5)";; sV*) WNUM="if3(abs(LepGood_pdgId)==13,0.45,0.75)";;
        esac
        case $WP in
            0??iRun3v1.0*) 
                       IDEmu="LepGood_idEmu3"
                       MUEXTRA="LepGood_jetRelIso < 0.5 && LepGood_jetBTagDeepFlav < smoothBFlav_custom(0.9*LepGood_pt*(1+LepGood_jetRelIso), 20, 45, btagcutL, btagcutM)  && LepGood_sip3d < smoothSIP(0.9*LepGood_pt*(1+LepGood_jetRelIso), 2.5,8.0, 15, 45)"; 
                       ELEXTRA="LepGood_jetRelIso < 1.0 && LepGood_mvaIso > 0.6"
                       SelDen="-A pt20 den '$SIP8 && $VDFM && (LepGood_mediumId || abs(LepGood_pdgId) == 11)  && PV_ndof > 100 && (LepGood_mvaTTH_run3 > $WNUM || (abs(LepGood_pdgId)==13 && $MUEXTRA) || (abs(LepGood_pdgId)==11 && $ELEXTRA))'"; Num="mvaPt_${WP%%i*}"i; XVar="mvaPt${WP%%i*}";;
            0??iRun3v3.0*) 
                       IDEmu="LepGood_idEmu3"
                       MUEXTRA="LepGood_jetRelIso < 0.5 && LepGood_jetBTagDeepFlav < smoothBFlav_custom(0.9*LepGood_pt*(1+LepGood_jetRelIso), 20, 45, btagcutL, btagcutM)  && LepGood_sip3d < smoothSIP(0.9*LepGood_pt*(1+LepGood_jetRelIso), 2.5,8.0, 15, 45)"; 
                       ELEXTRA="LepGood_jetRelIso < 0.9 && LepGood_mvaIso > 0.6"
                       SelDen="-A pt20 den '$SIP8 && $VDFM && (LepGood_mediumId || abs(LepGood_pdgId) == 11)  && PV_ndof > 100 && (LepGood_mvaTTH_run3 > $WNUM || (abs(LepGood_pdgId)==13 && $MUEXTRA) || (abs(LepGood_pdgId)==11 && $ELEXTRA))'"; Num="mvaPt_${WP%%i*}"i; XVar="mvaPt${WP%%i*}";; 
        esac
        case $WP in
            *f30*) SelDen="$SelDen -A pt20 ptfden '(LepGood_mvaTTH_run3 > $WNUM || 1/(1+LepGood_jetRelIso) > 0.30)' " ;;
            *f40*) SelDen="$SelDen -A pt20 ptfden '(LepGood_mvaTTH_run3 > $WNUM || 1/(1+LepGood_jetRelIso) > 0.40)' " ;;
            *f45*) SelDen="$SelDen -A pt20 ptfden '(LepGood_mvaTTH_run3 > $WNUM || 1/(1+LepGood_jetRelIso) > 0.45)' " ;;
            *f50*) SelDen="$SelDen -A pt20 ptfden '(LepGood_mvaTTH_run3 > $WNUM || 1/(1+LepGood_jetRelIso) > 0.50)' " ;;
            *f60*) SelDen="$SelDen -A pt20 ptfden '(LepGood_mvaTTH_run3 > $WNUM || 1/(1+LepGood_jetRelIso) > 0.60)' " ;;
            *f65*) SelDen="$SelDen -A pt20 ptfden '(LepGood_mvaTTH_run3 > $WNUM || 1/(1+LepGood_jetRelIso) > 0.65)' " ;;
            *j40*) SelDen="$SelDen -A pt20 ptfden '(LepGood_mvaTTH_run3 > $WNUM || LepGood_jetRelIso < 0.40)' " ;;
            *j50*) SelDen="$SelDen -A pt20 ptfden '(LepGood_mvaTTH_run3 > $WNUM || LepGood_jetRelIso < 0.50)' " ;;
            *j60*) SelDen="$SelDen -A pt20 ptfden '(LepGood_mvaTTH_run3 > $WNUM || LepGood_jetRelIso < 0.60)' " ;;
            *j70*) SelDen="$SelDen -A pt20 ptfden '(LepGood_mvaTTH_run3 > $WNUM || LepGood_jetRelIso < 0.70)' " ;;
            *j80*) SelDen="$SelDen -A pt20 ptfden '(LepGood_mvaTTH_run3 > $WNUM || LepGood_jetRelIso < 0.80)' " ;;
            *j90*) SelDen="$SelDen -A pt20 ptfden '(LepGood_mvaTTH_run3 > $WNUM || LepGood_jetRelIso < 0.90)' " ;;
        esac
	case $WP in
	    ...
	    *X5*) SelDen="$SelDen -A pt20 vcsvle '(LepGood_mvaSUSY > ${WNUM} && LepGood_mediumMuonId>0) || (${VCSVL} && ${ELEMVAPRESEL} && $PTF30)'"; Num="${Num%%X*}"; XVar="${XVar%%X*}";;
        ...
        case $WP in
            ...
            *E2ptc30) IDEMu="LepGood_idEmu2 || LepGood_pt*if3(LepGood_mvaTTH_run3>${WNUM}, 1.0, 0.90*(1+LepGood_jetRelIso)) < 30" ;;
	...
	case $WP in
	    ...
	    *ptJ95*)    ptJI="ptJI95";;
        ...
        B0="$B0 --legend=TR --showRatio --ratioRange 0.00 1.99   --yrange 0 1.0 " 
...
#	break;
	...
	#break;
        ...
        for C in 15_17.5 17.5_22.5 20_30 45_60 45_999; do
            conePtCut="-A pt20 conePt '${C%_*} < LepGood_pt*if3(LepGood_mvaTTH_run3>${WNUM},1,0.9*(1+LepGood_jetRelIso)) && LepGood_pt*if3(LepGood_mvaTTH_run3>${WNUM},1,0.9*(1+LepGood_jetRelIso)) < ${C#*_}' ";
